...
def exercise09():
    ...
    #r = requests.get('https://api.coindesk.com/v1/bpi/historical/close.json').json()  #returns a dictionary
    #df = pd.DataFrame(r)
    ...
    trace0 = go.Figure(go.Scatter(x = df.index, y = df['bpi']))
    trace0.update_layout(title_text='Bitcoin Closing Prices from 9/1/2017 to 10/5/2018')
    trace0.update_yaxes(title_text='Closing Price')
    #py.plot(trace0, filename = 'bitcoin-timeseries')

    plotly_url = py.plot(trace0, filename = 'bitcoin-timeseries')
...
def exercise13(n):
    ...
    s = pd.Series(np.random.randint(0,n, size =n), index = index)
...
def exercise15():
    ...
    df = pd.read_csv('https://raw.githubusercontent.com/JuliaData/CSV.jl/master/test/testfiles/Sacramentorealestatetransactions.csv')[['street','zip']].iloc[::5]
