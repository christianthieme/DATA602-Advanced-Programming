...
consists of:
    ...
    - Value: The float amount of Pandas Coinss transferred
...
class PandasChain:
    ...
    def __init__(self, name): 
        self.__name = str(name).upper()# Convert name to upper case and store it here
        self.__chain = list() # Create an empty list
    ...
    # It is private and therefore not publicly accessible. It will change the block status to committed, obtain the merkle
    ...
    def __commit_block(self,block): 
        ...
        root = block.get_simple_merkle_root()
        block.set_block_hash(hashlib.sha256(str(str(self.__prev_hash) + str(self.__id) + str(dt.datetime.now()) + str(self.__sequence_id) + str(np.random.randint(low = 1, high = 99)) + str(root)).encode('utf-8')).hexdigest())
        # Add code here
        self.__prev_hash = block.get_block_hash()
...
class Block:
    ...
    def display_header(self): 
        print(f"Sequence ID: {self.__seq_id}, status: {self.__status}, block hash: {self.__block_hash}, previous hash: {self.__prev_hash}, merkle hash: {self.__merkle_tx_hash}, transactions: {self.__transactions.shape[0]}")
    ...
    def get_simple_merkle_root(self): 
        single_hash = "".join(x for x in self.__transactions['TxHash'])
        self.__merkle_tx_hash =  hashlib.sha256(str(single_hash).encode('utf-8')).hexdigest()
