...
import chart_studio
chart_studio.tools.set_credentials_file(username='cthieme', api_key='fsuTH0oafjYgWWGHoGfb')
...
def exercise02(new_observations):
    ...
    iris_predictions = iris['target_names'][predictions]
...
def exercise03(neighbors,split):
    ...
    #knn.predict(X_test) don't need when using score
    knn_score = knn.score(X_test, y_test) #runs predictions on the full test dataset and returns the accuracy
    # ------ Place code above here /\ /\ /\ ------
...
def exercise04():
    ...
    # ------ Place code below here \/ \/ \/ ------
    #good resource for plotly https://plotly.com/python/getting-started-with-chart-studio/
    training_scores = []
    testing_scores = []

    X_train, X_test, y_train, y_test = tts(iris['data'], iris['target'], test_size = 0.30, random_state = 41, stratify = iris['target'])
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x = np.arange(1,31), y = testing_scores, name = 'train'))
    fig.add_trace(go.Scatter(x = np.arange(1,31), y = training_scores, name = 'test'))
    fig.update_layout(
    title='KNN Hyperparameter Tuning Neighbors 1-30', 
    xaxis_title = 'Neighbors',
    yaxis_title ='Score')
    #py.plot(trace0, filename = 'bitcoin-timeseries')

    data = [train_accuracy, test_accuracy]

    plotly_overfit_underfit_curve_url = py.plot(fig, filename = 'KNN Neighbors Model Selection')
...
def exercise05():
    ...
    df1 = pd.DataFrame(boston.target, columns = ['MEDV'])
    average_age = df['AGE'].mean()
    average_medv = df1['MEDV'].mean()
    medv_as_numpy_values = df1.values
...
def exercise06():
    ...
    plt.scatter(x = df["PTRATIO"], y = target['MEDV'])
    X_ptratio = target['MEDV'].values
...
def exercise07():
    ...
    reg.fit(df['PTRATIO'].values.reshape(-1,1), target["MEDV"].values.reshape(-1,1))
    prediction_space = np.linspace(min(df['PTRATIO']),max(df['PTRATIO'])).reshape(-1,1)
    reg_model = reg.predict( prediction_space )

    plt.scatter(x = df["PTRATIO"], y = target['MEDV'])
    plt.plot(prediction_space, reg_model, color = 'black')
    plt.show()
