...
class Box:
    ...
    def __eq__(self, other):
        return ((self.__length == other.__length) & (self.__width == other.__width))
...
class MangoDB:
    ...
    def display_all_collections(self):
        for collection in self.dictionary:
            print(f"collection: {collection}")
            for key,value in self.dictionary[collection].items():
                print(f"{key} {value}")


    def add_collection(self, collection_name):
        self.dictionary[collection_name] = dict()

    def update_collection(self, collection_name, updates):
        self.dictionary[collection_name].update(updates)

        ## update() method adds element(s) to the dictionary if the key is not in the dictionary. 
        # If the key is in the dictionary, it updates the key with the new value.

    def remove_collection(self, collection_name): 
        del self.dictionary[collection_name]

    def list_collections(self):
        print(list(self.dictionary.keys()))

    def get_collection_size(self, collection_name):
        return len(self.dictionary[collection_name])

    def to_json(self, collection_name):
        return json.dumps(self.dictionary[collection_name])
...
def exercise02():
    ...
    be called a collection, similar to the terminology used by MongoDB, an inferior version of MangoDB ;) A collection is a series of 
    2nd level key/value paires. The root value key is the name of the collection and the value is another dictionary containing arbitrary 
    ...
    The above is a representation of a dictionary of dictionaries. Default and temperatures are dictionaries or collections.
     The default collection has a series of key/value pairs that make up the collection. The MangoDB class should create only the 
     default collection, as shown, on instantiation including a randomly generated uuid using the uuid4() method and have the following 
     methods:
        - display_all_collections() which iterates through every collection and prints to screen each collection names and the collection's
         content underneath and may look something like:
            ...
            collection: temperatures
                1 50
                2 100 
        - add_collection(collection_name) allows the caller to add a new collection by providing a name. The collection will be empty but 
        will have a name.
    ...
    # ------ Place code below here \/ \/ \/ ------
    mdb = MangoDB()
    mdb.add_collection('test_scores')
    test_scores = {i:test_scores[i] for i in range(1,len(test_scores))}
    mdb.update_collection('test_scores', test_scores)
    print(mdb.get_collection_size('test_scores'))
...
def exercise03():
    ...
    # ------ Place code below here \/ \/ \/ ------
    response = requests.get('https://raw.githubusercontent.com/SmilodonCub/DATA602/master/avocado.csv')
   # with open(response.text.strip().split('\n')) as csv_file:
